    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
        while True:
            ...
            for attempt in range(_RETRIES):
                try:
                    ...
                    if resp.status_code == 429:
                        import asyncio
                        logger.warning("Polymarket rate-limited, backing off %ss", backoff * 3)
            ...
            if resp is None or not resp.is_success:
                logger.error("Failed to fetch Polymarket page at offset=%d — stopping", offset)
                break

            page = resp.json()
            # Gamma API may return a list directly or a dict with a "markets" key
            if isinstance(page, list):
                raw_markets = page
            elif isinstance(page, dict):
                raw_markets = page.get("markets") or page.get("data") or []
            else:
                raw_markets = []
